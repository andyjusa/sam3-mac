...
_DEFAULT_HF_CACHE = Path(__file__).resolve().parents[1] / ".cache" / "huggingface"
...
def download_ckpt_from_hf(version="sam3"):
    ...
    if version == "sam3.1":
        ...
        ckpt_name = "sam3.pt"
        cfg_name = "config.json"
    cache_dir = os.getenv("SAM3_HF_CACHE", str(_DEFAULT_HF_CACHE))
    _ = hf_hub_download(repo_id=repo_id, filename=cfg_name, cache_dir=cache_dir)
    checkpoint_path = hf_hub_download(
        repo_id=repo_id, filename=ckpt_name, cache_dir=cache_dir
    )
